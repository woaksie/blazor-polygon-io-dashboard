...
{
    public TickerResultsDto(string ticker, string name, string market, string locale, string primaryExchange,
        string type, bool active, string currencyName, string? cik, string? compositeFigi, string? shareClassFigi,
        double marketCap, string? phoneNumber, AddressDto? address, string? description, string? sicCode,
        string? sicDescription, string? tickerRoot, string? homepageUrl, int? totalEmployees, string listDate,
        BrandingDto? branding, long? shareClassSharesOutstanding, long? weightedSharesOutstanding)
    {
        Ticker = ticker;
        Name = name;
        Market = market;
